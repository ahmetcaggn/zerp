...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query("SELECT e FROM Employee e WHERE e.employeeCode = :employeeCode AND e.isDeleted = false")
    Optional<Employee> findByEmployeeCodeAndNotDeleted(@Param("employeeCode") String employeeCode);
    ...
    @Query("SELECT CASE WHEN COUNT(e) > 0 THEN true ELSE false END FROM Employee e WHERE e.employeeCode = :employeeCode AND e.isDeleted = false")
    boolean existsByEmployeeCodeAndNotDeleted(@Param("employeeCode") String employeeCode);
    ...
    @Query("SELECT e FROM Employee e WHERE e.isActive = true AND e.isDeleted = false")
    ...
    @Query("SELECT e FROM Employee e WHERE e.role = :role AND e.isDeleted = false")
    List<Employee> findByRoleAndNotDeleted(@Param("role") Role role);
    ...
    @Query("SELECT e FROM Employee e WHERE e.isActive = true AND e.status = :status AND e.isDeleted = false")
