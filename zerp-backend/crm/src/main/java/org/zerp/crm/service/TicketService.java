...
public class TicketService implements IResourceService<TicketResponse, TicketResponse,
    ...
    public Page<TicketResponse> findWithFilters(Map<String, String> filters, Pageable pageable) {
        ...
        Map<String, String> tenantScopedFilters = new HashMap<>();
        if (filters != null) {
            tenantScopedFilters.putAll(filters);
        }
        tenantScopedFilters.put("tenantId.eq", tenantId.toString());

        Specification<TicketEntity> specification = ticketSpecificationBuilder.build(tenantScopedFilters);
