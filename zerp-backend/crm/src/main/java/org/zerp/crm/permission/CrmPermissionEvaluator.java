...
public class CrmPermissionEvaluator {    private static final Set<PermissionTargetType> CRM_TARGET_TYPES = EnumSet.of(
    ...
    public boolean canReadTeam(UUID userId, TeamTarget target) {
        if (commonPermissionService.isAdminAny(userId, target.tenantId())) {
            log.debug("User {} is admin for tenant {}, granting read access", userId, target.tenantId());
            return true;
        }
    ...
    public boolean canCreateTeam(UUID userId, TenantParent parent) {
        if (commonPermissionService.isAdminAny(userId, parent.tenantId())) {
            log.debug("User {} is admin for tenant {}, granting create access", userId, parent.tenantId());
            return true;
        }
        return hasTeamHierarchyPermission(
    ...
    public boolean canUpdateTeam(UUID userId, TeamTarget target) {
        if (commonPermissionService.isAdminAny(userId, target.tenantId())) {
            log.debug("User {} is admin for tenant {}, granting update access", userId, target.tenantId());
            return true;
        }
        return hasTeamHierarchyPermission(
    ...
    public boolean canDeleteTeam(UUID userId, TeamTarget target) {
        if (commonPermissionService.isAdminAny(userId, target.tenantId())) {
            log.debug("User {} is admin for tenant {}, granting delete access", userId, target.tenantId());
            return true;
        }
        return hasTeamHierarchyPermission(
    ...
    public boolean canReadTicketHistory(UUID userId, TicketParent parent) {
        if (commonPermissionService.isAdminAny(userId, parent.tenantId())) {
            log.debug("User {} is admin for tenant {}, granting read access", userId, parent.tenantId());
            return true;
        }
        return hasTicketChildHierarchyPermission(
                userId, PermissionAction.READ_TICKET_HISTORY, PermissionTargetType.TICKET_HISTORY,
