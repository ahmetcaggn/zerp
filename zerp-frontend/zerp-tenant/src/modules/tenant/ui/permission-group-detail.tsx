...
function buildForm(group: PermissionGroupResponseDto): FormState {
  return {
    name: group.name,
...
export function PermissionGroupDetail({ groupKey }: Props) {
  ...
  function handleDelete() {
    ...
    deleteGroup(resolvedGroup.id, {
      onSuccess: () => {
        ...
        router.push(withLocale(locale, ROUTES.permissionGroups) as Route)
      },
    ...
    <Box sx={{ display: 'grid', gap: 2 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Button
          ...
          onClick={() => router.push(withLocale(locale, ROUTES.permissionGroups) as Route)}
        >
          {t('common.back')}
        </Button>
