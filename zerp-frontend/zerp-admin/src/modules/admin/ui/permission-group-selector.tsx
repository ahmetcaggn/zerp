...
function toSelection(group: PermissionGroupResponse): PermissionGroupSelectionValue {
  return {
    source: group.source,
    id: group.id,
    code: group.code,
...
export function PermissionGroupSelector({
  ...
  const { data: custom = [], isLoading: isCustomLoading } = useCustomPermissionGroups(tenantId, !disabled)
        ...
        <>
          ...
          <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
            {value.actions.map((action) => (
              <Chip key={action} label={prettifyPermissionEnumName(action)} size="small" variant="outlined" />
            ))}
          </Stack>

          <Alert severity="info">{t('permissionGroups.snapshotInfo')}</Alert>
        </>
      )}
    </Box>
  )
