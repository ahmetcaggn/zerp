# openapi_sale.api.ProductUpdateProductApi

## Load the API package
```dart
import 'package:openapi_sale/api.dart';
```

All URIs are relative to *https://zerpapi.femrek.dev*

Method | HTTP request | Description
------------- | ------------- | -------------
[**updateProduct**](ProductUpdateProductApi.md#) | **PUT** /sale/products/{id} | Update: Update an existing entity


# **updateProduct**
> ApiResponseProductDTO updateProduct(id, productUpdateDTO)

Update: Update an existing entity

Updates an existing entity with the provided field values. Implements ra-spring-data-provider's update operation with support for partial updates. Only the fields provided in the request body will be updated. 

### Example
```dart
import 'package:openapi_sale/api.dart';

final api_instance = ProductUpdateProductApi();
final id = 1; // String | Unique identifier of the entity to update
final productUpdateDTO = ProductUpdateDTO(); // ProductUpdateDTO | 

try {
    final result = api_instance.updateProduct(id, productUpdateDTO);
    print(result);
} catch (e) {
    print('Exception when calling ProductUpdateProductApi->updateProduct: $e\n');
}
```

### Parameters

Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **id** | **String**| Unique identifier of the entity to update | 
 **productUpdateDTO** | [**ProductUpdateDTO**](ProductUpdateDTO.md)|  | 

### Return type

[**ApiResponseProductDTO**](ApiResponseProductDTO.md)

### Authorization

No authorization required

### HTTP request headers

 - **Content-Type**: application/json
 - **Accept**: application/json

[[Back to top]](#) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to Model list]](../README.md#documentation-for-models) [[Back to README]](../README.md)

