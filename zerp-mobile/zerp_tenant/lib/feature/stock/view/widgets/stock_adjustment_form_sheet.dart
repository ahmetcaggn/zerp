...
class _AdjItem {
  ...
  String? resourceId;
  StockAdjustmentItemDTODirectionEnum direction =
      StockAdjustmentItemDTODirectionEnum.DECREASE;
...
class StockAdjustmentFormSheet extends StatefulWidget {
  ...
  @override
  State<StockAdjustmentFormSheet> createState() =>
      _StockAdjustmentFormSheetState();
}
...
class _StockAdjustmentFormSheetState extends State<StockAdjustmentFormSheet> {
  ...
  final _items = <_AdjItem>[_AdjItem()];
  bool _loading = false;
  ...
  Future<void> _submit() async {
    ...
    try {
      final dtoItems = _items
          ...
          .map(
            (item) => StockAdjustmentItemDTO(
              ...
              reason: item.reasonCtrl.text.trim(),
      ...
      await context.read<CubitStockOperations>().createAdjustment(
        ...
        builder: (context, scrollController) => Form(
          ...
          child: ListView(
            ...
            children: [
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          DropdownButtonFormField<
                            StockAdjustmentItemDTODirectionEnum
                          >(
                            initialValue: item.direction,
                            decoration: InputDecoration(
                              labelText: t.operation.direction,
                              border: const OutlineInputBorder(),
                            ),
                            items: StockAdjustmentItemDTODirectionEnum.values
                                .map((
                                  e,
                                ) {
                                  final key =
                                      'stock.operation.directions.${e.value}';
                                  return DropdownMenuItem(
                                    value: e,
                                    child: Text(
                                      (context.t[key] as String?) ?? e.value,
                                    ),
                                  );
                                })
                                .toList(),
                            onChanged: (v) =>
                                setState(() => item.direction = v!),
                          ),
                          const SizedBox(height: 8),
                          TextFormField(
              ...
              OutlinedButton.icon(
                onPressed: () => setState(() => _items.add(_AdjItem())),
